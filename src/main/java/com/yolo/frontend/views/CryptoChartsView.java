    ...
    private void cargarDatos() {
        new Thread(() -> {
            try {
                ...
                List<Double> precios = mapper.convertValue(root.get("precios"), new TypeReference<>() {});
                List<Double> sma = mapper.convertValue(root.get("sma"), new TypeReference<>() {});
                List<Double> ema = mapper.convertValue(root.get("ema"), new TypeReference<>() {});
                List<Double> rsi = mapper.convertValue(root.get("rsi"), new TypeReference<>() {});
                List<Double> macd = mapper.convertValue(root.get("macd"), new TypeReference<>() {});
                List<Double> signal = mapper.convertValue(root.get("signal"), new TypeReference<>() {});
