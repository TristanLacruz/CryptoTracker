...
public class GraficoRSIView extends VBox {
    ...
    /**
     * Constructor de la clase GraficoRSIView.
     * Inicializa el gráfico y carga los datos del RSI para la criptomoneda especificada.
     *
     * @param cryptoId El ID de la criptomoneda para la que se desea mostrar el gráfico RSI.
     */
    public GraficoRSIView(String cryptoId) {
        ...
        labelContainer = new HBox(ejeXLabel);
            ...
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    if (rsiActual >= 0) {
                        ...
                        Circle rsiDot = new Circle(6, javafx.scene.paint.Color.web("#448AFF"));
                        Label rsiLeyenda = new Label("RSI");
                        rsiLeyenda.setStyle("-fx-text-fill: #00FF00; -fx-font-family: Consolas; -fx-font-size: 12px;");
                        HBox leyendaBox = new HBox(6, rsiDot, rsiLeyenda);
                        leyendaBox.setAlignment(Pos.CENTER_LEFT);
                        leyendaBox.setPadding(new Insets(5, 0, 0, 10));
                        int labelContainerIndex = getChildren().indexOf(labelContainer);
                        getChildren().add(labelContainerIndex, leyendaBox); 
                    }
                });
            } catch (Exception e) {
